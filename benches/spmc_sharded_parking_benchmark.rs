...
fn benchmark(c: &mut Criterion) {
    ...
    // ==================== PUSH LATENCY ====================
    let mut group = make_group(c, "spmc/sharded_parking/push_latency");
    ...
    {
        let mut group = make_group(c, "spmc/sharded_parking/throughput/payload_8");
                ...
                group.bench_function(
                    ...
                    |b| {
                        b.iter(|| {
                            ...
                            let mut sent = 0;
                            let total_to_send = messages_per_receiver * receiver_count;
                            while sent < total_to_send {
                                let buf = tx.write_buffer();
                                let len = buf.len().min(total_to_send - sent);
                                if len == 0 {
                                    spin_loop();
                                    continue;
                                }
                ...
                group.bench_function(
                    ...
                    |b| {
                        b.iter(|| {
                            ...
                            let mut handles = Vec::with_capacity(receiver_count);
                            for i in 0..receiver_count - 1 {
                                let mut rx_clone = rx.clone().unwrap();
                                handles.push(spawn(move || {
                                    for j in 0..messages_per_receiver {
                                        black_box(rx_clone.recv());
                                        black_box(i + j);
                                    }
                                }));
                            ...
                            for j in 0..(messages_per_receiver * receiver_count) {
                                tx.send(black_box(Payload1024::new(j as u8)));
                            }
